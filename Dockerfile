...
ENV APACHE_SPARK_SITE="https://archive.apache.org/dist/"
...
RUN if [ -z "${scala_version}" ]; then \
    wget -qO "spark.tgz" "${APACHE_SPARK_SITE}spark/spark-${APACHE_SPARK_VERSION}/spark-${APACHE_SPARK_VERSION}-bin-hadoop${HADOOP_VERSION}.tgz"; \
  else \
    wget -qO "spark.tgz" "${APACHE_SPARK_SITE}spark/spark-${APACHE_SPARK_VERSION}/spark-${APACHE_SPARK_VERSION}-bin-hadoop${HADOOP_VERSION}-scala${scala_version}.tgz"; \
  fi && \
...
COPY "corelibs/odbc/sqlserverjdbc.jar"  "/home/jars/sqlserver/sqlserverjdbc.jar"
USER ${NB_UID}
